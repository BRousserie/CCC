...
namespace CCC
{
	...
	{
		CCC_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		CCC_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized)
		{
			int success = glfwInit();
			CCC_CORE_ASSERT(success, "Could not initialize GLFW !");
